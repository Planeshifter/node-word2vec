...
module.exports = function loadModel( file, callback ) {
	...
	fs.open(file, 'r', function(status, fd) {
	    ...
	    // need enough data to cover the string for of two long longs
		...
		// maximum length of a vector is maximum length of the vocabulary (50)
		// and size*4 bytes for the floats
		buffer = new Buffer(50+size*4);
		fs.read(fd, buffer, 0, 50+size*4, pos, function readOne(err, num) {
		    if (err) { callback(err, null); return }
		    ...
		    var word = buffer.toString().split(" ")[0];
		    size = arr[1];
		    pos += word.length+1;
		    pos += size*4+1;
		    var values = new Float32Array(buffer, word.length+1, size);

		    var o = new WordVec(word, values.slice(0,size))
		    var a;
		    var len = 0;
                    for ( a = 0; a < size; a++ ) {
			len += o.values[a] * o.values[a];
