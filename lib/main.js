...
var loadModel = require('./model.js');
...
module.exports = function(){
  ...
  */
  ...
  function word2vec(input, output, params){
    ...
    var word2vec = spawn("./word2vec",["-train", input,"-output", output].concat(paramsArr),{cwd: src_folder});
    word2vec.stdout.on('data', function (data) {
      charm.foreground("cyan")
      charm.write(data);
    });

    word2vec.on('close', function (code) {
      charm.write('\n');
  ...
  */
  function word2phrase(input, output, params){
    var charm = require('charm')();
    charm.pipe(process.stdout);
    charm.reset();
    ...
    var word2phrase = spawn("./word2phrase",["-train", input,"-output", output].concat(paramsArr),{cwd: src_folder});
    word2phrase.stdout.on('data', function (data) {
      charm.foreground("cyan")
      charm.write(data);
    });

    word2phrase.on('close', function (code) {
      charm.write('\n');
      charm.foreground("yellow");
      charm.write('Child process exited with code ' + code);
      charm.write('\n');
